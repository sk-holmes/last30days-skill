"""Perplexity Agent API and Search API integration.

The Perplexity source is paid and opt-in. Normal synthesis uses a controlled
Agent API request with only web search enabled. Deep Research remains an
explicit user action and uses an Agent API background run with the dynamic
high preset. OpenRouter is not an authorization fallback for this source.
"""
...
PERPLEXITY_MODE_SONAR = "sonar"  # Deprecated input alias for agent.
...
def _provider(config: dict[str, Any]) -> str | None:
    api_key = _config_text(config, "PERPLEXITY_API_KEY")
    return api_key or None


def _mode(config: dict[str, Any], deep: bool) -> str:
    if deep:
...
def _safe_request(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for key in ("model", "preset", "max_steps", "background"):
        if key in payload:
    ...
    artifact.update(
        {
            ...
            "provider": "perplexity",
            "mode": mode,
            "endpoint": "agent-background" if deep else (
                "search" if mode == PERPLEXITY_MODE_SEARCH else "agent"
            ),
            "deep": deep,
            "query": query,
        }
...
def search(
    ...
    """
    api_key = _provider(config)
    if not api_key:
        _log("PERPLEXITY_API_KEY is required for the Perplexity source, skipping")
        return [], {}

    mode = _mode(config, deep)
    try:
        ...
        return _agent_search(query, date_range, config, api_key, deep)
    except http.HTTPError as exc:
        if exc.status_code == 401:
            _log("Invalid Perplexity API key (401)")
        elif exc.status_code == 429:
            _log("Rate limited by Perplexity (429)")
        else:
            _log(f"HTTP error: {exc}")
        return [], _top_level_failure(query, mode, deep, exc)
